...
use super::{layout::{segment_status_table::{MAX_SEGMENTS, MAX_SEGMENT_SIZE}, Crc32, FlashRepr, Kind, Signature, SlotHeader, WriteExtStatus, DATA_REGION_OFFSET}, ManagerError, ScratchRam, Slot};
...
impl Slot {
    ...
    pub async fn is_valid_firmware<T: SpiFlash>(
        ...
        let header = self.load_header(flash, scratch).await?.ok_or(ManagerError::UnexpectedMissingHeader)?;
    ...
    pub(crate) async fn crc_valid<T: SpiFlash>(
        ...
        flash
            .read_to(self.idx * self.size + DATA_REGION_OFFSET, &mut scratch.firmware_rd_scratch[..Crc32::SIZE + Signature::SIZE])
            .await?;
        let (expected_crc32, later) = Crc32::take_from_bytes(&scratch.firmware_rd_scratch).ok_or(ManagerError::Fatal)?;
